function result = build_zhishan_cable_accel_polished_min90_display_pick()
%BUILD_ZHISHAN_CABLE_ACCEL_POLISHED_MIN90_DISPLAY_PICK Build stricter display pick.
%   Display-only candidate. Formal spectrum/force calculation remains
%   daily_median + [-100,100] m/s^2.
...
outputDir = fullfile(stableDir, 'polished_min90_display_pick');
...
% Chosen from the previous display grid search: lowest RMS30 max while
% keeping at least about 90 percent of valid data.
thresholdAbs = containers.Map( ...
    {'CF-1','CF-2','CF-3','CF-4','CF-5','CF-6','CF-7','CF-8'}, ...
    {50, 75, 5, 5, 50, 10, 50, 75});
...
for i = 1:numel(points)
    pointId = points{i};
    fprintf('polished min90 display %s\n', pointId);
    ...
    th = thresholdAbs(pointId);
    seg = segmentPct(pointId);
    ...
    finalIdx = find(strcmp(finalRules.PointID, pointId), 1);
    if isempty(finalIdx)
        error('Missing final rule row for %s.', pointId);
    end
    finalKeepPct = finalRules.KeepPct(finalIdx);
    finalRms30Max = finalRules.RMS30Max(finalIdx);
    rmsDelta = finalRms30Max - metric.rmsMax;
    rmsDeltaPct = 100 * rmsDelta / max(finalRms30Max, eps);
    keepDelta = keepPct - finalKeepPct;
    pass = keepPct >= minKeepPct && isfinite(metric.rmsMax);

    strategy = strategyText(th, seg, segmentMinutes);
    rationale = sprintf('min90 visual polish; RMS30 max delta %.2f (%.1f%%), keep delta %.2f%%', ...
        rmsDelta, rmsDeltaPct, keepDelta);
    plotPaths{i} = plotPoint(plotDir, pointId, binCenters, metric, th, seg, ...
        keepPct, finalRms30Max, rmsDeltaPct, pass);
    rows(end+1, :) = {pointId, strategy, th, seg, baseCount, keepPct, ...
        metric.rmsMax, metric.rmsP95, finalKeepPct, finalRms30Max, ...
        keepDelta, rmsDelta, rmsDeltaPct, pass, rationale, plotPaths{i}}; %#ok<AGROW>
end

manifest = cell2table(rows, 'VariableNames', { ...
    'PointID','Strategy','ThresholdAbsMps2','SegmentFilterTopPctRMS30', ...
    'BaseFiniteCount','KeepPct','RMS30Max','RMS30P95','FinalKeepPct', ...
    'FinalRMS30Max','KeepDeltaVsFinalPct','RMS30MaxDeltaVsFinal', ...
    'RMS30MaxDeltaVsFinalPct','AcceptancePass','Rationale','PlotPath'});

manifestXlsx = fullfile(outputDir, 'CableAccelPolishedMin90_manifest.xlsx');
manifestCsv = fullfile(outputDir, 'CableAccelPolishedMin90_manifest.csv');
contactSheet = buildReviewBoard(plotPaths, points, outputDir, 'CableAccelPolishedMin90_ContactSheet.jpg');
reviewBoard = contactSheet;
...
writeHtml(htmlPath, manifest, contactSheet, minKeepPct);
writeReadme(readmePath, manifest, minKeepPct);
...
fprintf('polished min90 html %s\n', htmlPath);
fprintf('polished min90 manifest %s\n', manifestXlsx);
fprintf('polished min90 pass %d\n', result.pass);
disp(manifest(:, {'PointID','Strategy','KeepPct','RMS30Max','FinalRMS30Max','RMS30MaxDeltaVsFinalPct','AcceptancePass'}));
end
...
function plotPath = plotPoint(plotDir, pointId, binCenters, metric, thresholdAbs, ...
        segmentPct, keepPct, finalRms30Max, rmsDeltaPct, pass)
...
fillBand(ax1, binCenters, metric.p25, metric.p75, [0.34 0.62 0.82], 0.70, '25%~75%');
plot(ax1, binCenters, metric.p50, 'Color', [0 0.18 0.42], 'LineWidth', 1.3, 'DisplayName', 'median');
hold(ax1, 'off');
grid(ax1, 'on');
grid(ax1, 'minor');
ylabel(ax1, 'm/s^2');
title(ax1, sprintf('%s polished min90 | abs<=%.0f | segment %.0f%% | keep %.2f%%', ...
    pointId, thresholdAbs, segmentPct, keepPct), 'Interpreter', 'none');
...
yline(ax2, finalRms30Max, '--', 'final RMS30 max', 'Color', [0.45 0.45 0.45], ...
...
plotPath = fullfile(plotDir, sprintf('CableAccelPolishedMin90_%s.jpg', pointId));
...
function writeHtml(path, manifest, contactSheet, minKeepPct)
fid = fopen(path, 'w', 'n', 'UTF-8');
cleaner = onCleanup(@() fclose(fid));
fprintf(fid, '<!doctype html>\n<html lang="zh-CN">\n<head>\n<meta charset="utf-8">\n');
fprintf(fid, '<title>Zhishan Cable Acceleration Polished Min90 Candidate</title>\n');
fprintf(fid, '<style>');
...
fprintf(fid, '.note{background:white;border-left:4px solid #9b5c00;padding:10px 12px;margin:14px 0 20px;color:#4b5563;line-height:1.7;}');
...
fprintf(fid, '.pass{background:#eaf7f2}.fail{background:#fff1f2}.figure{background:white;border:1px solid #d7dce2;padding:12px;margin:14px 0 24px;} .grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(520px,1fr));gap:14px;} img{max-width:100%%;height:auto;display:block;margin:auto;}');
fprintf(fid, '</style>\n</head>\n<body>\n');
fprintf(fid, '<h1>Zhishan Cable Acceleration Polished Min90 Candidate</h1>\n');
fprintf(fid, '<div class="note">Display-only stricter candidate. Minimum keep target: %.1f%%. Formal spectrum/force calculation remains daily_median + [-100,100] m/s^2.</div>\n', minKeepPct);
fprintf(fid, '<h2>Manifest</h2><table>\n');
fprintf(fid, '<tr><th>Point</th><th>Strategy</th><th>Keep %%</th><th>RMS30 max</th><th>Final RMS30 max</th><th>RMS30 delta %%</th><th>Pass</th></tr>\n');
for i = 1:height(manifest)
    cls = 'fail';
    if manifest.AcceptancePass(i), cls = 'pass'; end
    fprintf(fid, '<tr class="%s"><td>%s</td><td>%s</td><td class="num">%.3f</td><td class="num">%.3f</td><td class="num">%.3f</td><td class="num">%.1f</td><td class="num">%d</td></tr>\n', ...
        cls, htmlText(manifest.PointID{i}), htmlText(manifest.Strategy{i}), ...
...
function writeReadme(path, manifest, minKeepPct)
fid = fopen(path, 'w', 'n', 'UTF-8');
cleaner = onCleanup(@() fclose(fid));
fprintf(fid, '# Zhishan Cable Acceleration Polished Min90 Candidate\n\n');
fprintf(fid, '- Display-only stricter candidate.\n');
fprintf(fid, '- Minimum keep target: `%.1f%%`\n', minKeepPct);
fprintf(fid, '- Open `index.html` for visual review.\n');
fprintf(fid, '- Manifest: `CableAccelPolishedMin90_manifest.xlsx`\n\n');
fprintf(fid, 'Formal spectrum/force calculation remains `daily_median + [-100,100] m/s^2`.\n\n');
fprintf(fid, '| Point | Strategy | Keep %% | RMS30 max | Final RMS30 max | Delta %% | Pass |\n');
fprintf(fid, '|---|---|---:|---:|---:|---:|---:|\n');
for i = 1:height(manifest)
    fprintf(fid, '| %s | %s | %.3f | %.3f | %.3f | %.1f | %d |\n', ...
        manifest.PointID{i}, manifest.Strategy{i}, manifest.KeepPct(i), ...
        manifest.RMS30Max(i), manifest.FinalRMS30Max(i), ...
        manifest.RMS30MaxDeltaVsFinalPct(i), manifest.AcceptancePass(i));
end
end
