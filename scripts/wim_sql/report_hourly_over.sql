...
DECLARE @HourlyCriticalWeight FLOAT = $(HourlyCriticalWeight);
...
N'INSERT INTO #t (hh, Gross_Load)
  SELECT DATEPART(HOUR, HSData_DT), Gross_Load
...
SELECT
    b.bin_id,
    b.label,
    over_cnt = SUM(CASE WHEN t.Gross_Load >= @HourlyCriticalWeight THEN 1 ELSE 0 END)
FROM #bins AS b
LEFT JOIN #t AS t
       ON t.hh >= b.lo
