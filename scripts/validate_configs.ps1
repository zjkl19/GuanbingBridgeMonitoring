...
try {
    ...
    $code = "addpath(pwd, fullfile(pwd,'config')); paths=$matlabList; for i=1:numel(paths), fprintf('Validating %s\n', paths{i}); cfg=load_config(paths{i}); r=bms.config.SchemaValidator.validateDetailed(cfg); fprintf('  status=%s warnings=%d errors=%d\n', r.status, numel(r.warnings), numel(r.errors)); if strcmp(r.status,'failed'), error('Config validation failed: %s', paths{i}); end, end"
    matlab -batch $code
