function result = export_zhishan_cable_accel_current_best_report_images()
%EXPORT_ZHISHAN_CABLE_ACCEL_CURRENT_BEST_REPORT_IMAGES Export current-best images.
%   Copies the accepted auto-knee/current-best candidate into a neutral
%   report-facing folder. No formal calculation is recomputed or modified.
...
for i = 1:numel(points)
    ...
    if isempty(idx)
        error('Missing current-best row for %s.', pointId);
    end
    ...
    rows(end+1, :) = {pointId, sourceManifest.SelectedSource{idx}, ...
        ...
        sourceManifest.RMS30P95(idx), sourceManifest.KeepDeltaVsFinalPct(idx), ...
...
manifest = cell2table(rows, 'VariableNames', { ...
    ...
    'RMS30MaxImprovementPct','AcceptancePass','Rationale','PlotPath'});

contactSheetPath = fullfile(outputDir, 'CableAccelCurrentBestReport_ContactSheet.jpg');
copyfile(fullfile(autoKneeDir, 'CableAccelAutoKnee_ContactSheet.jpg'), contactSheetPath, 'f');

manifestPath = fullfile(outputDir, 'CableAccelCurrentBestReport_manifest.xlsx');
manifestCsvPath = fullfile(outputDir, 'CableAccelCurrentBestReport_manifest.csv');
...
fprintf('current-best report output dir %s\n', outputDir);
fprintf('manifest %s\n', manifestPath);
fprintf('html %s\n', htmlPath);
disp(manifest(:, {'PointID','SelectedSource','ThresholdAbsMps2', ...
    'SegmentFilterTopPctRMS30','KeepPct','RMS30Max','AcceptancePass'}));
end

function writeReadme(path, manifest, contactSheetPath)
fid = fopen(path, 'w', 'n', 'UTF-8');
cleaner = onCleanup(@() fclose(fid));
fprintf(fid, '# Zhishan Cable Acceleration Current-Best Report Images\n\n');
fprintf(fid, '- Open `index.html` for visual review.\n');
fprintf(fid, '- Manifest: `CableAccelCurrentBestReport_manifest.xlsx`\n');
...
fprintf(fid, '| Point | Selected | |x| threshold | RMS30 top %% | Keep %% | RMS30 max | Improve %% | Pass | Image |\n');
fprintf(fid, '|---|---|---:|---:|---:|---:|---:|---:|---|\n');
for i = 1:height(manifest)
    fprintf(fid, '| %s | %s | %.3f | %.1f | %.3f | %.3f | %.1f | %d | `%s` |\n', ...
        ...
        manifest.RMS30MaxImprovementPct(i), manifest.AcceptancePass(i), ...
...
fprintf(fid, '<title>Zhishan Cable Acceleration Current-Best Report Images</title>\n');
fprintf(fid, '<style>');
...
fprintf(fid, '</style>\n</head>\n<body>\n');
fprintf(fid, '<h1>&#33437;&#23665;&#22823;&#26725;&#32034;&#21147;&#21152;&#36895;&#24230;&#24403;&#21069;&#26368;&#20339;&#25253;&#21578;&#22270;</h1>\n');
fprintf(fid, '<div class="note">&#24403;&#21069;&#25512;&#33616;&#20351;&#29992;&#24050;&#39564;&#25910;&#30340; auto-knee &#20505;&#36873;&#65306;&#21482;&#25910;&#32039; CF-3/CF-4/CF-5&#65292;&#20854;&#20313;&#28857;&#20445;&#25345; balanced final&#12290;&#27491;&#24335;&#39057;&#35889;/&#32034;&#21147;&#35745;&#31639;&#19981;&#21464;&#12290;</div>\n');
fprintf(fid, '<h2>&#35268;&#21017; / Rules</h2><table>\n');
fprintf(fid, '<tr><th>Point</th><th>Selected</th><th>|x| threshold</th><th>RMS30 top %%</th><th>Keep %%</th><th>RMS30 max</th><th>Improve %%</th><th>Pass</th></tr>\n');
for i = 1:height(manifest)
    fprintf(fid, '<tr class="%s"><td>%s</td><td>%s</td><td class="num">%.3f</td><td class="num">%.1f</td><td class="num">%.3f</td><td class="num">%.3f</td><td class="num">%.1f</td><td class="num">%d</td></tr>\n', ...
        ...
        manifest.RMS30Max(i), manifest.RMS30MaxImprovementPct(i), ...
        manifest.AcceptancePass(i));
end
fprintf(fid, '</table>\n');
