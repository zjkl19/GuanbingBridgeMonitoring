function batch_rename_csv(root_dir, start_date, end_date)
% batch_rename_wave_csv 批量重命名“波形”和“特征值”子文件夹中的CSV文件
...
if est_time/60 > 3
    c = input(sprintf('预计重命名时间 %.1f 分钟，超过3分钟，是否继续？(y/n): ', est_time/60), 's');
    if ~strcmpi(c,'y')
        disp('已取消');
        return;
    end
