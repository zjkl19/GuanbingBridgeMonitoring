classdef test_run_request < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function finalAsyncStatusReflectsFailedManifest(tc)
            ...
            stoppedPath = fullfile(tc.TempDir, 'stopped_manifest.json');
            writeUtf8(failedPath, '{"status":"failed"}');
            writeUtf8(okPath, '{"status":"completed"}');
            writeUtf8(stoppedPath, '{"status":"stopped"}');
            ...
            tc.verifyEqual(bms.app.RunRequestRunner.finalStatusForManifest(stoppedPath), 'stopped');
            tc.verifyEqual(bms.app.RunRequestRunner.finalStatusForManifest( ...
                fullfile(tc.TempDir, 'missing.json')), 'failed');
        end

        function terminalAsyncProgressUsesManifestAuthority(tc)
            manifestPath = fullfile(tc.TempDir, 'terminal_manifest.json');
            moduleSteps = struct( ...
                'key', {'temperature','humidity'}, ...
                'label', {'Temperature','Humidity'}, ...
                'index', {1,2}, ...
                'status', {'running','pending'}, ...
                'stage', {'read_cache','pending'}, ...
                'current_point_id', {'TEMP-01',''}, ...
                'current_date', {'2026-04-05',''}, ...
                'processed_dates', {5,0}, ...
                'total_dates', {30,0}, ...
                'elapsed_seconds', {9.5,0}, ...
                'message', {'',''});
            moduleResults = struct( ...
                'key', {'temperature','humidity'}, ...
                'label', {'Temperature','Humidity'}, ...
                'status', {'stopped','skip'}, ...
                'message', {'User requested stop','Skipped after stop request'}, ...
                'elapsed_sec', {10,0});
            bms.core.Logger.writeJson(manifestPath, struct( ...
                'status', 'stopped', ...
                'module_steps', moduleSteps, ...
                'module_results', moduleResults));

            details = bms.app.RunRequestRunner.terminalProgressForManifest( ...
                manifestPath, 'stopped');

            tc.verifyEqual(details.progress_schema_version, 2);
            tc.verifyEqual(details.progress_authority, 'analysis_manifest');
            tc.verifyEqual(details.status, 'stopped');
            tc.verifyEqual({details.module_steps.status}, {'stopped','skipped'});
            tc.verifyEqual(details.module_steps(1).current_point_id, 'TEMP-01');
            tc.verifyEqual(details.module_steps(1).current_date, '2026-04-05');
            tc.verifyEqual(details.module_steps(1).processed_dates, 5);
            tc.verifyEqual(details.completed_modules, 2);
        end

        function terminalManifestCannotPassWithMissingPlannedModule(tc)
            manifestPath = fullfile(tc.TempDir, 'incomplete_terminal_manifest.json');
            moduleResults = struct( ...
                'key', {'config','temperature'}, ...
                'label', {'Config','Temperature'}, ...
                'status', {'warn','ok'}, ...
                'message', {'non-module warning',''}, ...
                'elapsed_sec', {0,2});
            bms.core.Logger.writeJson(manifestPath, struct( ...
                'status', 'completed', ...
                'enabled_modules', {{'temperature','humidity'}}, ...
                'module_results', moduleResults));

            details = bms.app.RunRequestRunner.terminalProgressForManifest( ...
                manifestPath, 'completed');

            tc.verifyEqual(details.status, 'failed');
            tc.verifyEqual(details.progress_authority, 'analysis_manifest');
            tc.verifyEqual({details.module_steps.key}, {'temperature','humidity'});
            tc.verifyEqual({details.module_steps.status}, {'completed','pending'});
            tc.verifyEqual(details.missing_module_keys, {'humidity'});
            tc.verifySubstring(details.message, 'omitted 1 planned module');
        end

        function explicitEmptyV2PlanIgnoresDiagnosticModuleRecords(tc)
            manifestPath = fullfile(tc.TempDir, 'empty_v2_plan_manifest.json');
            diagnosticRecords = struct( ...
                'key', {'config','offset_correction_report'}, ...
                'label', {'Config','Offset correction report'}, ...
                'status', {'warn','ok'}, ...
                'message', {'large config warning',''}, ...
                'elapsed_sec', {0,1});
            bms.core.Logger.writeJson(manifestPath, struct( ...
                'status', 'ok', ...
                ...
                'module_steps', {{}}, ...
                'enabled_modules', {{}}, ...
                'module_results', diagnosticRecords));

            details = bms.app.RunRequestRunner.terminalProgressForManifest( ...
                manifestPath, 'completed');

            tc.verifyEqual(details.status, 'completed');
            tc.verifyEqual(details.progress_authority, 'analysis_manifest');
            tc.verifyEqual(details.module_total, 0);
            tc.verifyEqual(details.completed_modules, 0);
            tc.verifyEmpty(details.module_steps);
            tc.verifyEmpty(details.missing_module_keys);
        end
