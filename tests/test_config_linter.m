classdef test_config_linter < matlab.unittest.TestCase
    methods (Test)
        ...
        function linterAcceptsValidDynamicRawModuleOverrides(tc)
            cfg = minimal_config();
            cfg.plot_common.dynamic_raw_modules.acceleration = struct( ...
                ...
                'gap_mode', 'connect');
            cfg.plot_common.dynamic_raw_modules.cable_accel = ...
                cfg.plot_common.dynamic_raw_modules.acceleration;

            result = bms.config.ConfigLinter.lint(cfg);

            tc.verifyFalse(any(contains(result.warnings, ...
                'plot_common.dynamic_raw_modules')));
        end

        function linterWarnsInvalidDynamicRawModuleOverrides(tc)
            cfg = minimal_config();
            cfg.plot_common.dynamic_raw_modules.acceleration = struct( ...
                'sampling_mode', 'full', 'line_width', 0.1, ...
                'render_mode', 'dense_band', 'gap_mode', 'join', ...
                'full_render_policy', 'lossy', ...
                'render_max_points', 0, ...
                'min_points_per_day', NaN, ...
                'unknown_field', true);
            cfg.plot_common.dynamic_raw_modules.wind = struct( ...
                'sampling_mode', 'full');

            result = bms.config.ConfigLinter.lint(cfg);

            tc.verifyTrue(any(contains(result.warnings, ...
                'acceleration.line_width must be a finite number')));
            tc.verifyTrue(any(contains(result.warnings, ...
                'acceleration.render_mode must be line when')));
            tc.verifyTrue(any(contains(result.warnings, ...
                'acceleration.gap_mode must be connect or break')));
            tc.verifyTrue(any(contains(result.warnings, ...
                'acceleration.unknown_field is not supported')));
            tc.verifyTrue(any(contains(result.warnings, ...
                'acceleration.full_render_policy must be')));
            tc.verifyTrue(any(contains(result.warnings, ...
                'acceleration.render_max_points must be a positive')));
            tc.verifyTrue(any(contains(result.warnings, ...
                'acceleration.min_points_per_day must be a positive')));
            tc.verifyTrue(any(contains(result.warnings, ...
                'dynamic_raw_modules.wind is not a supported')));
            tc.verifyTrue(any(strcmp({result.issues.category}, ...
