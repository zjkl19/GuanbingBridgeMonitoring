classdef test_crack_analysis_pipeline < matlab.unittest.TestCase
    ...
    methods (Test)
        function crackPipelineWritesStatsAndPlotsWithTemperature(tc)
            ...
            tc.verifyGreaterThanOrEqual(numel(dir(fullfile(tc.Root, 'crack_point_plots', '*.fig'))), 1);
            tc.verifyGreaterThanOrEqual(numel(dir(fullfile(tc.Root, 'crack_group_plots', '*.fig'))), 1);
...
function cfg = crack_cfg()
    ...
    cfg.plot_styles = struct('crack', struct( ...
        ...
        'single_output_dir_crack', 'crack_point_plots', ...
        'group_output_dir_crack', 'crack_group_plots', ...
