# Bridge Health Monitoring Platform / 桥梁健康监测工作平台
...
PySide6 工作平台以增量方式统一项目选择、本机 MATLAB 任务启动、分析结果审核、
图件确认和报告生成。已经验证的 MATLAB 分析内核仍由独立后台进程执行；报告生成代码
已嵌入同一个工作平台运行时，并由同一个 EXE 启动后台 worker。统一安装包不再携带或弹出
...
dist/BridgeMonitoringWorkbench/桥梁健康监测工作平台.exe
```

电脑正在办公时可使用不抢占前台的离屏审查构建：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/build_workbench_exe.ps1 -OffscreenScreenshots
```

该模式会执行冻结 EXE、CLI、后台 Runner、内嵌报告及 16 个 Qt 离屏页面检查，
但清单会明确记录 `screenshot_mode=qt_offscreen` 和
`native_screenshot_smoke=false`，因此不能打正式发布 ZIP。正式发布前仍须在电脑空闲时
执行不带该开关的原生 Windows 截图构建。

The stable `v1.8.3` scope, safeguards and promotion checks are recorded in
`docs/releases/v1.8.3.md`; the previous `v1.8.2` baseline remains recorded in
`docs/releases/v1.8.2.md`.
稳定版 `v1.8.3` 的功能范围、安全门槛和发布检查见
`docs/releases/v1.8.3.md`；上一稳定基线 `v1.8.2` 仍记录在
`docs/releases/v1.8.2.md`。

发布目录会同时包含 MATLAB 编译 Runner、配置/模板和嵌入工作平台的报告运行时，
不再复制独立 `BridgeReportBuilder.exe`。`release_manifest.json` 记录 EXE
哈希、发布清单自身以外的文件数/总大小及启动自测结果；其中
`analysis_runner_failure_exit_smoke=true` 表示构建时已经实际证明：失败分析会先把
`failed` 状态及其 `manifest_path` 持久化，再让编译 Runner 以非零退出码结束。缺少
该证据的旧清单会被 GitHub Release 打包脚本拒绝。`workbench_startup.png`、
...
`workbench_plot_common_editor.png`、`workbench_spectrum_editor.png`、
`workbench_unzip_settings.png` 与
...
`BridgeMonitoringWorkbench_cli_error.log`；即使无控制台的 `stderr` 为空，或短生命周期父进程留下的
重定向句柄已经失效/关闭，也不会再弹出未处理异常。
...
滤波后二次清洗或绘图字段。上下限在表格和交互工具中最多显示、写入三位小数；未编辑的规则
仍保持原始 JSON 数值。测点专用规则提供三种明确的曲线操作：旧 MATLAB GUI 等价的
“拖线设置上下限”可自动加载当前任务的独立曲线记录，也可通过醒目的主入口直接选择任意可信
MATLAB `.fig`，并在同一曲线上同时调整两条水平线和共同时间窗；“框选设为下限”取框中实际有限样本的
最高值，严格删除更低值；“框选设为上限”取框中最低值，严格删除更高值。等于候选阈值的点保留，
矩形边界本身不会被当作阈值。对话框显示选中点数和基于抽样预览的预计删除数，取消或关闭不改表格，
确认后也只是尚未保存的内存修改；正式删除量仍以完整缓存重算结果为准。

直接 FIG 模式由同一 `BridgeAnalysisRunner.exe` 启动 MATLAB Runtime 原生拖线/框选窗口，读取 FIG
中的真实 X/Y 数据，而不是从 JPG/PNG 像素反推数值。外部 FIG 只作为当前选中配置行的数值参考，
默认保留该配置行原有时间窗；只有确认 FIG 与当前任务一致时才可显式采用拖线时间窗。滤波后二次清洗页
也保留任意 FIG 入口，但不会把滤波前的自动预览误当作滤波后曲线。

工作平台现已增加独立“生成当前测点曲线”轻量任务和“自动清洗建议（Beta）”子页。轻量任务只处理
当前模块和测点，优先读取有效 MAT 缓存，不采用现有清洗阈值，也不运行自动阈值算法；它发布独立的
`threshold_curve_preview` / `threshold_curve_record` 契约。普通用户从桥梁、月份、模块、测点和历史任务
列表导入跨任务曲线，不需要辨认 JSON；导入后可继续拖动上下限和紫色时间窗，且不会回写来源任务。
人工工具只接受新版独立曲线契约，旧 `auto_threshold_preview.json` 不再兼容，需要时重新运行轻量曲线任务。
两类后台任务都显示实际阶段、模块、测点、当前日期、已处理/总日期、百分比和耗时，停止时只写本任务
专属停止标志并等待安全边界。

主窗口会优先恢复仍合法的上次窗口位置；首次启动按当前屏幕可用区域约 90% 初始化并限制在屏幕内，
适配多屏和 100%/125%/150% 缩放。完成态审核采用签名缓存，悬停和滚动不会再每两秒同步重算全部
报告门禁。分析区保留“模块进度”总条，并显示完整模块步骤、当前测点/日期及完成/失败/跳过/停止状态；
耗时统一显示到秒，不暗示模块进度等于准确时间比例。

Beta 子页仍调用 MATLAB `AutoThresholdProposalService`，由重新编译的同一
`BridgeAnalysisRunner.exe` 在独立进程生成建议；PySide6 只负责参数、状态、人工复核和受保护写入。
建议不会自动落入配置，Runner 会在读取数据前校验请求固定的配置 SHA；只有勾选的
`range/window_range` 在二次确认、再次校验配置 SHA 和自动备份后写入。
Beta Runner 同时把最多 20000 点/测点的极值保留预览写成独立 JSON，并在建议结果中固定其
...
`segmented` 分段结构，并在写入前拒绝日期重叠；生效范围可通过日历精确到秒，表示修正
应用到哪些数据，并不另设一段基线参考样本。“组图配置”页直接管理
...
`dense_band`。缺口连接还可按分析模块或单个测点覆盖；运行时按“测点 > 模块 > 兼容旧模块
设置 > 全局”解析，连接方式与断线倍数可分别继承，只改变时程图在缺口处的画法，不改变
统计值。“频谱覆盖与找峰”页统一管理加速度/索力加速度频谱测点及默认、逐测点
...
正式报告审核。“报告生成”页不再打开第二个 GUI，而是由当前工作平台 EXE 启动同一内嵌
运行时的后台 worker，并显示预检、生成、质量检查和完成阶段。最终结果表展示 DOCX ZIP/主文档/媒体、可用 PDF 页数、报告内容
清单缺失项与警告；配置、模板和分析结果清单的固定 SHA 仍会在子进程
读取前再次校验。

水仙花月报生成器会先将经确认的数据完整性说明写入正文和生成清单，并在
Microsoft Word 启动前完成精确 OOXML/媒体 QC。统一报告 worker 以 DOCX-only
模式调用生成器，随后独占权威 Word 字段更新、PDF 导出和逐页视觉 QC；生成器
QC 未通过时不会启动 Word。独立 PowerShell 辅助脚本只用于人工诊断或恢复，
默认在同目录临时副本上工作并原子发布结果，不提供按模糊进程名强制结束 Word
的入口。大型文档完成字段和目录更新并关闭首个 Word 会话后，会在新的
PowerShell/COM 进程中通过 Word `SaveAs2` 导出 PDF，避免同一自动化会话中的
`ExportAsFixedFormat` 长时间卡死。
...
报告门禁区分不可人工放行的红色问题与必须逐项确认的黄色缺项。桥梁/日期/配置错配、
清单或哈希变化、分析失败、来源计数不闭合和图件绑定错误始终阻断。设备断采、明确无有效
数据或报告类型允许省略等项目，只有在当前报告适配器能安全清空模板旧媒体或写入“本期无
有效数据”时才会列为黄色；确认记录绑定当前分析清单 SHA，清单变化即失效。生成器第一次
发现新的报告构建缺项会以 `disclosure_required` 结束，确认后再次构建还必须得到完全相同的
缺项集合。最终交付状态为 `passed_with_disclosures`，正文、报告清单和 QC 会同时记录缺项
数量、原因与处置，不会伪装成无缺项通过。
...
`docs/pyside6_workbench_migration.md`. The unified PySide6 workbench is the
production entry; the legacy MATLAB GUI is retained only for rollback and
development compatibility while the MATLAB analysis kernel remains in use.
迁移设计与当前范围见 `docs/pyside6_workbench_migration.md`；统一 PySide6 工作平台现为
正式生产入口，旧 MATLAB GUI 仅保留用于回滚和开发兼容，MATLAB 分析内核继续使用。
...
Module failures are recorded in the final analysis manifest. The async status
keeps that failed manifest path for diagnosis, and the compiled Runner then
returns a non-zero process exit code; callers must check both the exit code and
the retained manifest instead of treating process completion as analysis
success. 模块失败会先写入最终分析清单，异步状态保留该失败清单路径供诊断，随后编译
Runner 以非零退出码结束；调用方必须同时检查退出码和保留的清单，不能把“进程已结束”
等同于“分析成功”。
...
Report generation is a background capability of `桥梁健康监测工作平台.exe`.
...
报告生成已成为 `桥梁健康监测工作平台.exe` 的后台能力。旧的独立报告窗口及其 EXE
...
- `workbench/` unified PySide6 workbench and job contract / 统一 PySide6 工作平台与任务契约
...
The manifest is versioned with `schema_version=3`. It records the normalized
run request and identity, preflight results, selected modules and their
`ok/fail/skip` states, data layout/source mode and coverage, timing/errors,
statistics and artifact inventories, source records, data-index summaries and
other report-facing validation evidence. This is the preferred
machine-readable source for later report-generation checks.
运行清单带 `schema_version=3`。它记录归一化运行请求和任务身份、预检结果、
所选模块及其 `ok/fail/skip` 状态、数据布局/数据源模式与覆盖范围、耗时和错误、
统计表与产物清单、源数据记录、数据索引摘要及其它面向报告验收的证据。
后续报告生成前检查应优先读取这个结构化文件。

运行中的 `analysis_status.json` 现支持 `schema_version=2` 的完整
`module_steps` 数组。每个计划模块独立记录待运行、运行中、已完成、失败、跳过或
已停止状态，以及可观测到的阶段、测点、日期、日期进度和耗时；旧标量状态文件仍按
保守口径兼容。工作平台的整体进度条表示“已进入终态的模块数/计划模块数”，明确不
是耗时百分比。运行结束后以最终 `analysis_manifest_*.json` 为权威，避免把运行期的
“已处理”误显示成“通过”。
...
解压并发由配置项 `preprocessing.unzip.max_workers` 控制，可在工作平台
“配置与预警值 → 解压并发”中选择自动、1、2、4 或自定义数量。旧配置中已有的
正整数保持兼容；缺失或 `null` 仍按安全串行值 1 处理；`"auto"` 为保守自动模式，
最多请求 2 个 MATLAB 工作进程。并行环境不可用或多个 ZIP 共享输出目录时会安全回退为
串行，并在解压摘要中记录请求值、解析值、实际值及回退原因。开发机可运行下列有上限的
合成 ZIP 基准（仅使用 `tempdir`，不接受生产数据根目录）：

```matlab
benchmark_archive_extract_workers('ArchiveCount', 4, ...
    'FilesPerArchive', 3, 'PayloadBytes', 65536, ...
    'WorkerSettings', {1, 'auto', 2, 4});
```
`preprocessing.unzip.summary_file` 若为绝对路径则原样使用，若为相对路径则相对于解压
`output_root` 解析，不能再次把绝对路径拼接到输出根之下。
...
`analysis_manifest_*.json` now uses `schema_version=3`. In addition to module
status and artifact counts, it binds the normalized run request, preflight and
data-layout/source evidence needed to reject a mismatched task before report
generation. Report builders should use these signed paths and records rather
than unbounded directory searches.
`analysis_manifest_*.json` 现使用 `schema_version=3`。除模块状态与产物计数外，
还绑定归一化运行请求、预检以及数据布局/数据源证据，用于在报告生成前拒绝
错配任务。报告生成器应优先使用这些已绑定路径和记录，不应依赖无界目录搜索。
...
Jiulongjiang supports its complete configured daily-export analysis set,
including guarded daily-ZIP extraction and raw MAT-cache prebuilding before the
15 applicable analysis modules.
九龙江项目已支持完整的日导出分析流程，包括安全解压、原始 MAT 缓存预生成以及当前配置中
适用的 15 个分析模块。
...
`config/path_profiles.json` 的存储位置方案处理：默认按电脑名自动匹配，也可在工作平台
手动选择配置组或为当前任务指定自定义路径。共享方案仅保留开发机、133 分析机和
126 数据机；办公室电脑已从存储位置选项删除，仅在运维机器清单中保留轻量远控记录。
...
- `v1.8.3`: adopts the bridge-neutral project name, hardens Jiulongjiang cache-aware acquisition disclosure, isolates large Word-to-PDF exports, and prevents invalid inherited CLI streams from opening an unhandled-exception dialog.
- `v1.8.3`：采用不再绑定单座桥梁的项目名，完善九龙江缓存来源披露，隔离大型 Word 转 PDF 进程，并避免失效的继承 CLI 输出句柄触发未处理异常窗口。
- `v1.8.2`: restores direct arbitrary MATLAB FIG drag/box threshold tools, hardens verified cache cleanup and large-data recovery, and closes Word report-number and page-field release gates.
- `v1.8.2`：恢复任意 MATLAB FIG 的拖线/框选阈值工具，强化已验证缓存清理与大数据恢复，并补齐 Word 报告编号和页码域发布门禁。
...
- `v1.8.0`：发布稳定版 PySide6 统一工作平台，保留 MATLAB 分析内核，并纳入多桥目录、机器路径配置组、受保护高级配置、编译 Runner 契约及闭合发布清单。
...
- `v1.8.1-rc1`: keeps Jiulongjiang monthly reports bound to the requested period, records optional patrol availability, and treats traffic/WIM as explicitly not applicable when configured.
- `v1.8.1-rc1`：九龙江月报严格绑定请求月份，记录可选巡查资料状态，并在配置声明后将交通/WIM 明确标记为不适用。
...
- `v1.8.0`：将报告生成内嵌到统一工作平台，并为水仙花、九龙江月报增加监测期、媒体及来源的失败即阻断校验。
