...
图件确认和报告生成条件检查。迁移期间，已经验证的 MATLAB 分析内核与
现有报告生成器仍作为独立后台保留。
...
当前桥梁未配置此类规则时，会明确说明“不是加载失败”并隐藏空表格。六桥冻结版矩阵要求
总览非空且不存在格式错误。
...
正式报告审核。“报告生成”页不再打开第二个 GUI，而是启动独立报告子进程并显示预检、
生成、质量检查和完成阶段。最终结果表展示 DOCX ZIP/主文档/媒体、可用 PDF 页数、报告内容
