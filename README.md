...
滤波后二次清洗或绘图字段。测点专用规则提供三种明确的曲线操作：旧 MATLAB GUI 等价的
“拖线设置上下限”可自动加载当前任务预览，也可直接选择任意可信 MATLAB `.fig`，并在同一曲线上同时调整两条水平线和共同时间窗；“框选设为下限”取框中实际有限样本的
...
迁移设计与当前范围见 `docs/pyside6_workbench_migration.md`；统一 PySide6 工作台现为
...
解压并发由配置项 `preprocessing.unzip.max_workers` 控制，可在工作台
