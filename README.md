...
滤波后二次清洗或绘图字段。测点专用规则还可分别使用“打开曲线预览并拖线设置下限（删除低于此值）”和
“打开曲线预览并拖线设置上限（删除高于此值）”：已有预览曲线支持拖线或精确输入，可限定时间窗、显示基于
预览样本的预计删除数并在保存前撤销；正式删除量仍以完整缓存重算结果为准。旧 MATLAB GUI
也保留单侧上下限，不再把只填一侧的有效规则静默丢弃。
...
- `v1.8.1-rc3`: generalizes optional raw-CSV cache prebuild across all configured bridges and the three supported data layouts, removes the office PC from storage profiles, clarifies drag-line threshold actions, and adds strict native Windows focus/icon/DPI release evidence.
- `v1.8.1-rc3`：将可选原始 CSV 缓存预生成扩展至所有已配置桥梁及三种数据布局，从存储方案删除办公室电脑，明确拖线阈值的删除方向，并增加严格的原生 Windows 焦点、图标和 DPI 发布证据。
