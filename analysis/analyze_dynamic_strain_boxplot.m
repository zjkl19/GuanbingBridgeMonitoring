...
function make_boxplot_and_stats(dataMat, labels, groupName, outdir, ds_cfg, tag, ts, dt0, dt1, cfg)
    ...
    title(sprintf('动应变箱线图（高通滤波后）%s [%s]', groupName, tag), 'Interpreter', 'none');
    ...
    bms.io.StatsWriter.writeModuleTableChecked(statsTbl, xlsxPath, 'dynamic_strain_highpass', 'Sheet', groupName);
end
...
function plot_timeseries_group(tsList, labels, groupName, outdir_ts, dt0, dt1, ds_cfg, ylim_group, tag, ts, cfg)
    ...
    title(sprintf('动应变时程（高通滤波后）%s [%s]', groupName, tag), 'Interpreter', 'none');
    grid on; grid minor;
