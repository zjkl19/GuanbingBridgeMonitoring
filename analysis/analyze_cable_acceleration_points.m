...
%   cfg: load_config() 结构
    ...
    for i = 1:numel(tpts)
        ...
        plot_accel_curve(root_dir, pid, times, vals, rec.mn, rec.mx, style, cfg);
...
function style = get_style(cfg, key)
    defaults = struct('ylabel','索力加速度 (m/s^2)', ...
...
out = fullfile(root_dir,'时程曲线_索力加速度'); bms.core.PathResolver.ensureDir(out);
...
fname = sprintf('CableAccelRMS10_%s_%s_%s', pid, datestr(xmin,'yyyymmdd'), datestr(xmax,'yyyymmdd'));
bms.plot.PlotService.saveModuleBundle(fig, out, [fname '_' ts], cfg);
end
