function batch_unzip_data_parallel(root_dir, start_date, end_date)
% batch_unzip_data_parallel 并行批量解压健康监测数据中的 ZIP 文件（使用 PowerShell）
%   batch_unzip_data_parallel(root_dir, start_date, end_date)
...
if est_time_s/60 > 3
    prompt = sprintf('预计解压 %.1f 分钟 %s，超过3分钟，是否继续？(y/n): ', est_time_s/60, env_note);
    c = input(prompt, 's');
    if ~strcmpi(c, 'y')
        disp('操作已取消');
        return;
    ...
    fprintf('预计解压 %.1f 分钟 %s，开始运行', est_time_s/60, env_note);
end
