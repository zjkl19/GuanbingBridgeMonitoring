classdef CrackAnalysisPipeline
    ...
    methods (Static)
        ...
        function ps = plotSpec(rootDir, style)
            ...
            crackSingleDir = bms.analyzer.StructuralPlotConfigService.getStyleField(style, 'single_output_dir_crack', '');
            if isempty(crackSingleDir)
                ps.crack_single_dir = ps.crack_dir;
            else
                ps.crack_single_dir = fullfile(rootDir, ...
                    bms.analyzer.StructuralPlotConfigService.sanitizeFilename(crackSingleDir));
            end
            crackGroupDir = bms.analyzer.StructuralPlotConfigService.getStyleField(style, 'group_output_dir_crack', '');
            if isempty(crackGroupDir)
                ps.crack_group_dir = ps.crack_dir;
            else
                ps.crack_group_dir = fullfile(rootDir, ...
                    bms.analyzer.StructuralPlotConfigService.sanitizeFilename(crackGroupDir));
            end
            ...
            for i = 1:numel(points)
                ...
                if ~isempty(S.crack_times)
                    bms.analyzer.CrackAnalysisPipeline.plotSingleCurve( ...
                        ...
                        ps.crack_single_dir, startDate, endDate, ...
            ...
            for gi = 1:numel(names)
                ...
                if ~isempty(crackLabels)
                    bms.analyzer.CrackAnalysisPipeline.plotGroupCurve( ...
                        ...
                        ps.crack_group_dir, groupName, startDate, endDate, ...
