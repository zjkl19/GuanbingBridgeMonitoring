    ...
    methods (Static)
        function specs = catalog()
            ...
            specs = [ ...
                ...
                S('cache_prebuild','doCachePrebuild','预生成分析缓存','', 'preprocess', 'BridgeScoped', false, 'PresetField', 'cache_prebuild', 'Description', ['读取已解压 CSV 并生成或复用 MAT 分析缓存；默认不删除源数据（保留 CSV）。仅在独立任务中明确确认后，' ...
                    '逐日删除已验证且可由原 ZIP 恢复的适用 CSV，原 ZIP、WIM、Excel 和未配置 CSV 保留。']), ...
                S('lowfreq_sync','doLowfreqSync','基康低频同步','', 'preprocess', 'BridgeScoped', false, 'GuiLabel', '基康低频同步', 'PresetField', 'lowfreq_sync'), ...
