...
for i = 1:length(files)
    ...
    try
        ...
        elseif method_choice == 2
            % 手动指定上下限
            lower_bound = -1000; % 手动指定下限值
upper_bound = 1000; % 手动指定上限值
        else
            disp('无效的选择，跳过该文件。');
            continue;
        end
        ...
        all_data = [all_data, strain_data]; % 合并所有文件的应变数据
...
% 绘制箱线图
figure;
boxplot(all_data, 'Labels', labels, 'LabelOrientation', 'inline');
title('测点应变数据箱线图比较');
xlabel('测点');
ylabel('应变（με）');

% 保存图像
output_dir = '箱线图结果_去离群值';
